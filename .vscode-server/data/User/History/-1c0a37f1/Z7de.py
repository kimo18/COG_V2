...
def main(opt,wandb_args):
    ...
    if opt.mode == "train":
        wandb.init(
                ...
                config=wandb_args,
                name="test "#"COG_lvl0_ Weighted_loss(joint+time) (not alignedV2)"#"COG_V4_lvl1_ Weighted_loss (alignedV2 (1, 0.9))(1,0.1,1))"
        )
        dataloader = torch.utils.data.DataLoader(dataset, batch_size=opt.batch_size, shuffle=True,num_workers=0, pin_memory=True)
        test_dataloader = torch.utils.data.DataLoader(test_dataset, batch_size=opt.batch_size, shuffle=False,num_workers=0, pin_memory=True)

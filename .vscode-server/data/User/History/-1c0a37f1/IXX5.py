...
def main(opt,wandb_args):
    ...
    if opt.mode == 'train':
        ...
        wandb.init(
                ...
                config=wandb_args,
                name="COG_lvl2_Linear( gumple joint contribution)_separate long_short"
        )
...
def run_model(nb_kpts, net_pred, batch_size, optimizer=None, data_loader=None, opt=None, epo=0):
    ...
    if opt.mode == 'train': #train
        ...
        for batch_idx, (x, y) in enumerate(data_loader): # in_n + kz
            ...
            data_out, mix_loss = net_pred(x_c, y_c,epo)
...
def eval(opt, net_pred, data_loader, nb_kpts, epo):
    ...
    stats = {}
    for j in eval_frame:
        e1, e2 = loss_list1[j]/(n/opt.batch_size)*1000, aligned_loss_list1[j]/(n/opt.batch_size)*1000

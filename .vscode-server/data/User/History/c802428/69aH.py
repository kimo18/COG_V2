    ...
    def forward(self, input_ori, gt, epoch):
        ...
        if mode == "adaptive":
            ...
            with torch.no_grad():
                ...
                print("this is the mean of each level",max_idx.float().mean(-1))
            ...
            entropy = -(attn * torch.log(attn + 1e-8)).sum(dim=-1).mean()  # scalar
            loss = self.mix_loss(predic, gt) + self.w_cb * k_loss + 0.01 * entropy

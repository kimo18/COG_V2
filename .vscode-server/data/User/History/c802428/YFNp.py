...
class IAFormer(nn.Module):
    def __init__(self, seq_len, d_model, opt, num_kpt, dataset, k_levels = 3 , share_d = False):
        ...
        self.attention_mlp = nn.Sequential(
            nn.Linear(k_levels, 128),
            nn.LayerNorm(128),             # Normalizes across features
            nn.ReLU(),
            nn.Dropout(dropout),            # Helps avoid overconfidence

            nn.Linear(128, 64),
            nn.ReLU(),
            nn.Dropout(dropout),

            nn.Linear(64, k_levels)         # Final logits over K levels
        )
    ...
    def forward(self, input_ori, gt, epoch):
        ...
        if mode == "adaptive":
            ...
            results_feat_filtered_results = [r[:,:,:,self.opt.frame_in:] for r in results_feat]
            time_idx = torch.stack(results_feat_filtered_results, dim=-1)
            ...
            def gumbel_tau_schedule(epoch, total_epochs, min_tau=0.5, max_tau=5):
                progress = epoch / 0.1*total_epochs

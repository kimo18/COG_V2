...
class IAFormer(nn.Module):
    def __init__(self, seq_len, d_model, opt, num_kpt, dataset, k_levels = 3 , share_d = False):
        ...
        self.attention_mlp = nn.Sequential(
            nn.Linear(2*self.k_levels +1, 128),
    ...
    def forward(self, input_ori, gt, epoch):
        ...
        if mode == "adaptive":
            ...
            # time_feat = time_idx.view(1, 1, -1, 1).expand(B, P, -1, 1)
            # print(time_feat.shape)
            # preds_aggregated = stacked_feats.mean(dim=3)
            # print(preds_aggregated.shape)

            # selector_input = torch.cat([l2norm_error, time_feat, preds_aggregated], dim=-1)
            print(selector_input.shape)
            ...
            attn_logits = self.attention_mlp(selector_input)  #(B,P,T, M)

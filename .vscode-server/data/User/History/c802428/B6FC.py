...
class IAFormer(nn.Module):
    ...
    def forward(self, input_ori, gt, epoch):
        ...
        squared_error = torch.norm((predic[:, :, self.opt.frame_in:, :] - gt[:, :, self.opt.frame_in:, :]), dim=3) # -> (B, P, T, J, M)
        error_per_level = squared_error.mean(dim=3)

            error_per_level = torch.mean(torch.norm((predic[:, :, self.opt.frame_in:, :] - gt[:, :, self.opt.frame_in:, :]), dim=3))

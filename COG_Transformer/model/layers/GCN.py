...
class GraphConvolution(nn.Module):
    ...
    def forward(self, input):
        # print(f"this is the GCN input shape {input.shape}")
        # print(f"this is the GCN weight shape {self.weight.shape}")

        support = torch.matmul(input, self.weight)
        # print(f"this is the GCN weight shape {support.shape}")
...
class GCN(nn.Module):
    ...
    def forward(self, x, is_output = False):
        y = self.gc1(x)
        b, n, f = y.shape
        y = self.bn1(y.view(b, -1)).view(b, n, f)
        y = self.act_f(y)
